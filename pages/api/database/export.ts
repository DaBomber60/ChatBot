...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (isJsonFormat) {
      // Legacy JSON format
      const filename = `homechatbot-export-${timestamp}.json`;
      ...
      const readmeContent = `HomeChatBot Database Export
Generated: ${new Date().toISOString()}
Format Version: ${exportData.version}

Contents:
- database.json: Complete database export in JSON format
- This file can be imported back into HomeChatBot
...
For support or questions, visit: https://github.com/DaBomber60/HomeChatBot
`;
      ...
      const filename = `homechatbot-export-${timestamp}.zip`;
